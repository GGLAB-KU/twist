...
ANNOTAION_NAME = "group5" # TODO: update
# get the parent directory
parent_dir = os.path.dirname(os.getcwd())
input_file = parent_dir+'/outputs/excel/' + ANNOTAION_NAME + '_results.xlsx'

df_all = pd.read_excel(input_file)
ANNOTATOR_MAILS = df_all['mail'].tolist()
...
def generate_task_page(pdf, task_number, df_all, annotator_mail):
    ...
    pdf.multi_cell(0, 10, f"  - Number of TN: {annotator_data.get(f'{task_prefix}_english_term_detection_tn', 'N/A')}")
    ...
    pdf.multi_cell(0, 10, f"  - Kappa: {annotator_data.get(f'{task_prefix}_english_term_detection_kappa', 'N/A'):.2%}")
    ...
    pdf.multi_cell(0, 10, f"  - Number of TN: {annotator_data.get(f'{task_prefix}_turkish_term_detection_tn', 'N/A')}")
    ...
    pdf.multi_cell(0, 10, f"  - Kappa: {annotator_data.get(f'{task_prefix}_turkish_term_detection_kappa', 'N/A'):.2%}")
    ...
    pdf.set_text_color(0, 0, 0)  # Black for regular text
    pdf.multi_cell(0, 10, f"  - Number of Intersection: {annotator_data.get(f'{task_prefix}_english_term_linking_intersection_num', 'N/A')}")
...
for mail in ANNOTATOR_MAILS:
    ...
    relevant_columns = [
        ...
        "cumulative_english_term_detection_kappa",
        ...
        "cumulative_turkish_term_detection_kappa",
    ...
    pdf.multi_cell(0, 10, f"  - Kappa: {summary_data.get('cumulative_english_term_detection_kappa', 'N/A'):.2%}")
    ...
    pdf.multi_cell(0, 10, f"  - Kappa: {summary_data.get('cumulative_turkish_term_detection_kappa', 'N/A'):.2%}")
    ...
    # Save the PDF file
    parent_dir = os.path.dirname(os.getcwd())
    pdf.output(parent_dir+'/outputs/pdf/' + pdf_file_name)
